...
function CreateUserModal({ onClose, onCreated }) {
  const [form, setForm] = useState({ userName: "", email: "", password: "", roleId: 2 });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width: "100%", maxWidth: "480px", boxShadow: "0 8px 40px rgba(0,0,0,0.18)",
      }} onClick={(e) => e.stopPropagation()}>

        <div className="d-flex justify-content-between align-items-center mb-4">
          <h5 className="mb-0 fw-bold" style={{ fontSize: "1.25rem" }}>Create New User</h5>
        ...
        {[
          ...
          <div className="mb-3" key={f.name}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.85rem" }}>{f.label}</label>
        ...
        <div className="d-flex justify-content-end gap-2">
          <button onClick={onClose} style={{
            ...
            borderRadius: "8px", padding: "8px 22px", cursor: "pointer", fontWeight: 500,
          }}>Cancel</button>
          <button onClick={handleSubmit} disabled={loading} style={{
            background: "#111", color: "#fff", border: "none",
            borderRadius: "8px", padding: "8px 22px", cursor: "pointer", fontWeight: 600,
          }}>
...
function ChangeRoleModal({ user, onClose, onChanged }) {
  ...
  const handleRoleChange = async (roleId) => {
    ...
    try {
      ...
      if (!res.ok) { setError("Failed to change role."); return; }
      onChanged();
      onClose();
    } catch {
      setError("Could not reach the server.");
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#fff", borderRadius: "16px", padding: "32px",
        width: "100%", maxWidth: "400px", boxShadow: "0 8px 40px rgba(0,0,0,0.18)",
      }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {ROLES.map((r) => (
            <button key={r.id} onClick={() => handleRoleChange(r.id)} disabled={saving}
              style={{
              ...
              <span>{r.name}</span>
              {user.role === r.name && <span style={{ fontSize: "0.75rem", opacity: 0.7 }}>current</span>}
            </button>
          ))}
        </div>
      </div>
...
export default function AdminPage() {
  ...
  const handleDeactivate = async (u) => {
    if (!window.confirm(`Deactivate ${u.userName}? They will not be able to log in.`)) return;
    const res = await authFetch(`${API_BASE_URL}/users/${u.id}/deactivate`, { method: "PUT" });
    if (res.ok) { showMsg(`${u.userName} has been deactivated.`); fetchUsers(); }
    else        { showMsg("Failed to deactivate user."); }
  };

  const handleActivate = async (u) => {
    const res = await authFetch(`${API_BASE_URL}/users/${u.id}/activate`, { method: "PUT" });
    if (res.ok) { showMsg(`${u.userName} has been activated.`); fetchUsers(); }
    else        { showMsg("Failed to activate user."); }
  };

  const handleDelete = async (u) => {
    if (!window.confirm(`Permanently delete ${u.userName}? This cannot be undone.`)) return;
    const res = await authFetch(`${API_BASE_URL}/users/${u.id}`, { method: "DELETE" });
    if (res.status === 409) {
      showMsg(`${u.userName} has attached data — use Deactivate instead.`);
    } else if (res.ok) {
      showMsg(`${u.userName} has been deleted.`);
      fetchUsers();
    } else {
  ...
  const tdStyle = {
    padding: "13px 16px", fontSize: "0.85rem",
  ...
  const sidebarLink = (tab) => ({
    ...
    fontSize: "0.9rem", border: "none", width: "100%",
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "#f8fafc", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: "flex", flex: 1 }}>
        ...
        <aside style={{
          ...
          <p style={{ fontSize: "0.7rem", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.08em", padding: "0 16px", marginBottom: "8px" }}>
            Navigation
          </p>
          <button style={sidebarLink("tickets")} onClick={() => setActiveTab("tickets")}>
            🎫 All Tickets
          </button>
          <button style={sidebarLink("users")} onClick={() => setActiveTab("users")}>
            👥 All Users
          </button>
        </aside>
        ...
        <main style={{ flex: 1, padding: "32px", overflow: "auto" }}>
          ...
          {activeTab === "tickets" && (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
                <div>
                  <h4 style={{ margin: 0, fontWeight: 700, fontSize: "1.3rem" }}>All Tickets</h4>
              ...
              {ticketLoad && <p style={{ color: "#6b7280" }}>Loading tickets...</p>}
                ...
                <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e5e7eb", overflow: "hidden" }}>
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr>
                          {["Ticket ID","Title","Status","Priority","Category","Assigned To","Submitted By","Created",""].map((h) => (
                      ...
                      <tbody>
                        {tickets.length === 0 ? (
                          ...
                          <tr key={t.id}
                            ...
                            <td style={{ ...tdStyle, fontWeight: 600, color: "#111", fontFamily: "monospace" }}>
                            ...
                            <td style={tdStyle}>
                              {t.assignedToName || <span style={{ color: "#9ca3af" }}>Unassigned</span>}
                            </td>
                            ...
                            <td style={tdStyle}>
                              <div style={{ display: "flex", gap: "6px" }}>
                                {actionBtn("#eff6ff", "#1d4ed8", "Activity", () => setActivityModal(t))}
                              </div>
                            </td>
                          </tr>
                        ))}
          ...
          {activeTab === "users" && (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
                <div>
                  <h4 style={{ margin: 0, fontWeight: 700, fontSize: "1.3rem" }}>All Users</h4>
                  <p style={{ margin: 0, color: "#6b7280", fontSize: "0.85rem" }}>
                ...
                <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e5e7eb", overflow: "hidden" }}>
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr>
                          {["ID","Username","Email","Role","Status","Created By","Created Date","Updated By","Updated Date","Actions"].map((h) => (
                            <th key={h} style={thStyle}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {users.length === 0 ? (
                          ...
                          <tr key={u.id}
                            onMouseEnter={(e) => e.currentTarget.style.background = "#fafafa"}
                            onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                            <td style={{ ...tdStyle, fontWeight: 600, color: "#111" }}>{u.id}</td>
                            ...
                            <td style={{ ...tdStyle, color: "#6b7280" }}>{u.createdBy || "—"}</td>
                            <td style={{ ...tdStyle, color: "#6b7280" }}>
                              {u.createdDate ? new Date(u.createdDate).toLocaleDateString() : "—"}
                            </td>
                            <td style={{ ...tdStyle, color: "#6b7280" }}>{u.updatedBy || "—"}</td>
                            <td style={{ ...tdStyle, color: "#6b7280" }}>
                              {u.updatedDate ? new Date(u.updatedDate).toLocaleDateString() : "—"}
                            </td>
                            {/* 👇 action buttons */}
                            <td style={tdStyle}>
                              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                                {actionBtn("#eff6ff", "#1d4ed8", "Role",
                                  () => setRoleModal(u))}
                                {u.isActive
                                  ? actionBtn("#fef9c3", "#854d0e", "Deactivate",
                                      () => handleDeactivate(u))
                                  : actionBtn("#d1fae5", "#065f46", "Activate",
                                      () => handleActivate(u))
                                }
                                {actionBtn("#fee2e2", "#b91c1c", "Delete",
                                  () => handleDelete(u))}
                              </div>
                            </td>
                          </tr>
      ...
      {showModal && (
        <CreateUserModal
          ...
          onCreated={() => { if (activeTab === "users") fetchUsers(); }}
        />
      )}
      ...
      {roleModal && (
        <ChangeRoleModal
          ...
          onChanged={() => { fetchUsers(); showMsg(`Role updated for ${roleModal.userName}.`); }}
        />
      )}
